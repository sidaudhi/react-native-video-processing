...
export class ProcessingManager {
  static trim(source: sourceType, options: trimOptions): Promise<string> {
    ...
    return TrimmerManager.trim(mData).then((res) => res.source);
  }
  ...
  static getPreviewForSecond(
    ...
    return TrimmerManager.getPreviewImageAtPosition(mData).then(
      (res) => res.image
    );
  }
  ...
  static getTrimmerPreviewImages(
    ...
    return TrimmerManager.getTrimmerPreviewImages(mData).then(
      (res) => res.images
    );
  }
  ...
  static compress(source: sourceType, options: any): Promise<*> {
    const actualSource: string = getActualSource(source);
    return TrimmerManager.compress(actualSource, options).then(
      (res) => res.source
    );
  }
  ...
  static boomerang(source: sourceType): Promise<*> {
    const actualSource: string = getActualSource(source);
    return TrimmerManager.boomerang(actualSource).then((res) => res.source);
  }

  static reverse(source: sourceType): Promise<*> {
    const actualSource: string = getActualSource(source);
    return TrimmerManager.reverse(actualSource).then((res) => res.source);
  }
  ...
  static crop(source: sourceType, options: cropOptions): Promise<string> {
    ...
    const actualSource: string = getActualSource(source);
    return TrimmerManager.crop(actualSource, options).then((res) => res.source);
  }
